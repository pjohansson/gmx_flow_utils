...
from gmx_flow import convert_gmx_flow_1_to_2, read_data, write_data
...
if __name__ == '__main__':
    ...
    for fn, fnout in get_files_from_range(
            ...
            ext=args.ext):
        data, info = read_data(fn)

        if info['format'] == 'GMX_FLOW_1':
            data, info = convert_gmx_flow_1_to_2(data, info, args.width)
        else:
            num_already_converted += 1

        backup_file(fnout)
        write_data(fnout, data, info)
